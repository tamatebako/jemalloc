...
For comprehensive profiling documentation including Alpine-specific setup, see
link:doc/profiling.adoc#alpine-linux-musl-libc[Profiling Guide § Alpine Linux].
...
==== ARM64 Speculation Barrier - Performance optimization
...
The ARM SB implementation on Linux was contributed by
@salvatoredipietro in
https://github.com/jemalloc/jemalloc/issues/2843[GitHub issue #2843].
(Thank you!)

**Performance improvement**: depends on the version of instruction set and CPU,
it is reported that AWS Graviton 3 shows approximately
~30% (https://github.com/jemalloc/jemalloc/issues/2843[GitHub issue #2843]), and
Apple Silicon M3 shows approximately ~11.3% improvement in benchmarks.

For detailed benchmark comparison and methodology, see
link:doc/profiling.adoc#armv9-speculation-barrier-optimization[Profiling Guide § ARMv8.5+ speculation barrier optimization].
...
NOTE: ARMv8.5-A introduced the SB instruction in 2019. Performance improvement
varies by microarchitecture - server-class ARM cores (Neoverse) typically show
larger gains than client cores (Apple Silicon).
...
./myapp  # Automatically uses SB on ARMv8.5-A+, ISB on older
----
...
sysctl -n hw.optional.arm.FEAT_SB  # Returns 1 if supported
----
...
# ID_AA64ISAR1_EL1 bits 11:8 should be nonzero
----
...
Verify at runtime (cross-platform):
...
For comprehensive profiling documentation including build instructions, usage
examples, configuration options, and platform-specific considerations, see
link:doc/profiling.adoc[profiling.adoc].
...
* link:doc/profiling.adoc[Profiling Guide] - Comprehensive heap profiling across all platforms
* link:doc_internal/PROFILING_INTERNALS.md[PROFILING_INTERNALS.md] - In-depth profiling implementation details
* link:doc/CMAKE_INTEGRATION.adoc[CMAKE_INTEGRATION.adoc] - Comprehensive CMake and vcpkg integration guide
* link:RELEASING.adoc[RELEASING.adoc] - Complete release process guide
* link:INSTALL.adoc[INSTALL.adoc] - Building and installation instructions
