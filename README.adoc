...
# Build and test (C++ disabled to avoid hanging OOM tests)
./autogen.sh
...
**C++ support note**: C++ integration is disabled on Alpine/musl CI testing
(`--disable-cxx`) because C++ memory exhaustion tests hang in containerized
environments due to differences in OOM behavior between musl and glibc. For
production deployments requiring C++ support, you can enable it with
`--enable-cxx` but be aware that memory exhaustion scenarios may behave
differently.
