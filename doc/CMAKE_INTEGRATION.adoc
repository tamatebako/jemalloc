...
|`JEMALLOC_BUILD_SHARED` |BOOL |ON |Build shared library
|`JEMALLOC_BUILD_STATIC` |BOOL |ON |Build static library
|`JEMALLOC_ENABLE_DOC` |BOOL |OFF |Enable documentation
|`JEMALLOC_ENABLE_STATS` |BOOL |ON |Enable statistics
|`JEMALLOC_ENABLE_PROF` |BOOL |OFF |Enable profiling
|`JEMALLOC_EXTRA_CONFIGURE_ARGS` |STRING |"" |Additional configure arguments
|===
...
cmake -B build \
    ...
    -DJEMALLOC_EXTRA_CONFIGURE_ARGS="--with-jemalloc-prefix=je_"
----

=== Custom Configure Flags

Pass additional flags to the autotools configure script using `JEMALLOC_EXTRA_CONFIGURE_ARGS`:
...
----
cmake -B build \
    -DJEMALLOC_EXTRA_CONFIGURE_ARGS="--with-jemalloc-prefix=je_ --with-lg-page=16"
cmake --build build
cmake --install build
----

**Via CMakeLists.txt (before add_subdirectory):**

[source,cmake]
----
# In your project's CMakeLists.txt
set(JEMALLOC_EXTRA_CONFIGURE_ARGS "--with-jemalloc-prefix=je_" CACHE STRING "" FORCE)
...
# Set configure args before declaring the content
set(JEMALLOC_EXTRA_CONFIGURE_ARGS "--with-malloc-conf=tcache:false" CACHE STRING "" FORCE)
...
**Common autotools configure flags:**

[cols="2,3",options="header"]
|===
|Flag |Description

|`--with-jemalloc-prefix=<prefix>` |Prefix all jemalloc symbols
|`--with-malloc-conf=<opts>` |Embedded runtime configuration
|`--with-lg-page=<n>` |Set page size to 2^n bytes
|`--with-lg-hugepage=<n>` |Set huge page size to 2^n bytes
|`--enable-debug` |Enable debug mode
|`--enable-prof-libunwind` |Use libunwind for profiling
